		...
		for (int i = 0; i < tempMove.getText().length(); i++) {
			...
			if (dir.equals("vertical")) {
				...
				for (int g = 0; g < gapIndex.size(); g++)
					if (i == gapIndex.get(g))
						tempMove.setSumScore(tempMove.getSumScore()
								+ board.getWordScore(tempMove.getLetters().get(i).getLoc(), dir, "vertical"));
				...
				if (board.hasNeighbor(tempMove.getLetters().get(i).getLoc())[1])
					tempMove.setSumScore(tempMove.getSumScore()
							+ board.getWordScore(tempMove.getLetters().get(i).getLoc(), dir, "horizontal"));
			}
			...
			if (dir.equals("single")) {
				int toAdd;
				toAdd = board.getWordScore(tempMove.getLetters().get(i).getLoc(), dir, "horizontal");
				if (toAdd > tempMove.getLetters().get(i).getValue())
					tempMove.setSumScore(tempMove.getSumScore() + toAdd);
				toAdd = board.getWordScore(tempMove.getLetters().get(i).getLoc(), dir, "vertical");
				if (toAdd > tempMove.getLetters().get(i).getValue())
					tempMove.setSumScore(tempMove.getSumScore() + toAdd);
			}
